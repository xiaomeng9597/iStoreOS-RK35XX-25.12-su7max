...
static const struct yt921x_info yt921x_infos[] = {
	{ "YT9215SC", YT9215_MAJOR, 1, 0, YT921X_PORT_MASK_INT0_n(5), YT921X_PORT_MASK_EXT0 | YT921X_PORT_MASK_EXT1, },
	{ "YT9215S", YT9215_MAJOR, 2, 0, YT921X_PORT_MASK_INT0_n(5), YT921X_PORT_MASK_EXT0 | YT921X_PORT_MASK_EXT1, },
	{ "YT9215RB", YT9215_MAJOR, 3, 0, YT921X_PORT_MASK_INT0_n(5), YT921X_PORT_MASK_EXT0 | YT921X_PORT_MASK_EXT1, },
	{ "YT9214NB", YT9215_MAJOR, 3, 2, YT921X_PORT_MASK_INTn(1) | YT921X_PORT_MASK_INTn(3), YT921X_PORT_MASK_EXT0 | YT921X_PORT_MASK_EXT1, },
	{ "YT9213NB", YT9215_MAJOR, 3, 3, YT921X_PORT_MASK_INTn(1) | YT921X_PORT_MASK_INTn(3), YT921X_PORT_MASK_EXT1, },
	{ "YT9218N", YT9218_MAJOR, 0, 0, YT921X_PORT_MASK_INT0_n(8), 0, },
	{ "YT9218MB", YT9218_MAJOR, 1, 0, YT921X_PORT_MASK_INT0_n(8), YT921X_PORT_MASK_EXT0 | YT921X_PORT_MASK_EXT1, },
	{}
};
...
/* Helper function to replace dsa_bridge_ports introduced in Linux 6.9 */
static u16 yt921x_bridge_ports_mask(struct dsa_switch *ds, struct net_device *br)
{
	struct dsa_port *dp;
	u16 mask = 0;
...
static int yt921x_reg_wait(struct yt921x_priv *priv, u32 reg, u32 mask, u32 *valp)
{
	...
	ret = read_poll_timeout(yt921x_reg_read, res, res || (val & mask) == *valp,
			       YT921X_POLL_SLEEP_US, YT921X_POLL_TIMEOUT_US, false,
			       priv, reg, &val);
	if (ret)
		return ret;
	if (res)
...
static int yt921x_extif_read(struct yt921x_priv *priv, int port, int reg, u16 *valp)
{
	...
	if (res)
		return res;
	mask = YT921X_MBUS_CTRL_PORT_M | YT921X_MBUS_CTRL_REG_M | YT921X_MBUS_CTRL_TYPE_M | YT921X_MBUS_CTRL_OP_M;
	ctrl = YT921X_MBUS_CTRL_PORT(port) | YT921X_MBUS_CTRL_REG(reg) | YT921X_MBUS_CTRL_TYPE_C22 | YT921X_MBUS_CTRL_READ;
...
static int yt921x_extif_write(struct yt921x_priv *priv, int port, int reg, u16 val)
{
	...
	if (res)
		return res;
	mask = YT921X_MBUS_CTRL_PORT_M | YT921X_MBUS_CTRL_REG_M | YT921X_MBUS_CTRL_TYPE_M | YT921X_MBUS_CTRL_OP_M;
	ctrl = YT921X_MBUS_CTRL_PORT(port) | YT921X_MBUS_CTRL_REG(reg) | YT921X_MBUS_CTRL_TYPE_C22 | YT921X_MBUS_CTRL_WRITE;
	res = yt921x_reg_update_bits(priv, YT921X_EXT_MBUS_CTRL, mask, ctrl);
	if (res)
		return res;
...
static int yt921x_read_mib(struct yt921x_priv *priv, int port)
{
	...
	pp->rx_frames = mib->rx_64byte + mib->rx_65_127byte + mib->rx_128_255byte +
			mib->rx_256_511byte + mib->rx_512_1023byte + mib->rx_1024_1518byte +
			mib->rx_jumbo;
	pp->tx_frames = mib->tx_64byte + mib->tx_65_127byte + mib->tx_128_255byte +
			mib->tx_256_511byte + mib->tx_512_1023byte + mib->tx_1024_1518byte +
			mib->tx_jumbo;
...
{
	struct yt921x_port *pp = container_of(work, struct yt921x_port, mib_read.work);
...
static void yt921x_dsa_get_eth_mac_stats(struct dsa_switch *ds, int port, struct ethtool_eth_mac_stats *mac_stats)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	struct yt921x_port *pp = &priv->ports[port];
	struct yt921x_mib *mib = &pp->mib;
...
static void yt921x_dsa_get_eth_ctrl_stats(struct dsa_switch *ds, int port, struct ethtool_eth_ctrl_stats *ctrl_stats)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	struct yt921x_port *pp = &priv->ports[port];
	struct yt921x_mib *mib = &pp->mib;
...
static const struct ethtool_rmon_hist_range yt921x_rmon_ranges[] = {
	{ 0, 64 }, { 65, 127 }, { 128, 255 }, { 256, 511 }, { 512, 1023 },
	{ 1024, 1518 }, { 1519, YT921X_FRAME_SIZE_MAX }, {}
};

static void yt921x_dsa_get_rmon_stats(struct dsa_switch *ds, int port, struct ethtool_rmon_stats *rmon_stats, const struct ethtool_rmon_hist_range **ranges)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	struct yt921x_port *pp = &priv->ports[port];
	struct yt921x_mib *mib = &pp->mib;
...
static void yt921x_dsa_get_stats64(struct dsa_switch *ds, int port, struct rtnl_link_stats64 *stats)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	struct yt921x_port *pp = &priv->ports[port];
	struct yt921x_mib *mib = &pp->mib;
	...
	stats->rx_errors = stats->rx_length_errors + stats->rx_over_errors + stats->rx_crc_errors + stats->rx_frame_errors;
	stats->tx_errors = stats->tx_aborted_errors + stats->tx_fifo_errors + stats->tx_window_errors;
	stats->rx_dropped = mib->rx_dropped;
	stats->multicast = mib->rx_multicast;
	stats->collisions = mib->tx_collisions;
}

static void yt921x_dsa_get_pause_stats(struct dsa_switch *ds, int port, struct ethtool_pause_stats *pause_stats)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	struct yt921x_port *pp = &priv->ports[port];
	struct yt921x_mib *mib = &pp->mib;
...
static int yt921x_set_eee(struct yt921x_priv *priv, int port, struct ethtool_eee *e)
{
...
static int yt921x_dsa_set_mac_eee(struct dsa_switch *ds, int port, struct ethtool_eee *e)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	int res;
...
static int yt921x_dsa_port_change_mtu(struct dsa_switch *ds, int port, int new_mtu)
{
	...
	mutex_lock(&priv->reg_lock);
	res = yt921x_reg_update_bits(priv, YT921X_MACn_FRAME(port), YT921X_MAC_FRAME_SIZE_M, YT921X_MAC_FRAME_SIZE(frame_size));
	mutex_unlock(&priv->reg_lock);
...
static u64 ldexpu64(u64 v, int e) { return e >= 0 ? v << e : v >> -e; }
...
static u32 burst2token(u64 burst, int unit, int C) { return ldexpu64(burst, -(2 * unit + C)); }
static u64 token2burst(u32 token, int unit, int C) { return ldexpu64(token, 2 * unit + C); }
...
static int yt921x_marker_tfm(struct yt921x_marker *marker, u64 rate, u64 burst, unsigned int flags, unsigned int slot_ns, u32 cir_max, u32 cbs_max, int unit_max, struct yt921x_priv *priv, int port, struct netlink_ext_ack *extack)
{
	const int C = flags & YT921X_MARKER_PKT_MODE ? YT921X_TOKEN_PKT_C : YT921X_TOKEN_BYTE_C;
	struct device *dev = to_device(priv);
	struct yt921x_marker m;
	u64 burst_est, burst_sug, burst_max, rate_max;
	...
	if (burst_sug > burst)
		NL_SET_ERR_MSG_FMT_MOD(extack, "Consider match rate %llu with burst at least %llu", rate, burst_sug);
...
static int yt921x_meter_config(struct yt921x_priv *priv, unsigned int id, const struct yt921x_marker *marker)
{
	u32 ctrls[3];
	ctrls[0] = 0;
	...
	update_ctrls_unaligned(&ctrls[0], &ctrls[1], YT921X_METER_CTRLab_EBS_M, YT921X_METER_CTRLab_EBS(marker->ebs));
	update_ctrls_unaligned(&ctrls[1], &ctrls[2], YT921X_METER_CTRLbc_CBS_M, YT921X_METER_CTRLbc_CBS(marker->cbs));
				       ...
				       struct dsa_mall_policer_tc_entry *policer)
{
	...
	res = yt921x_marker_tfm(&marker, policer->rate_bytes_per_sec, policer->burst,
				0, priv->meter_slot_ns, YT921X_METER_CIR_MAX,
...
{
	u32 mask = ingress ? YT921X_MIRROR_IGR_PORTn(port) : YT921X_MIRROR_EGR_PORTn(port);
	return yt921x_reg_clear_bits(priv, YT921X_MIRROR, mask);
}

static int yt921x_mirror_add(struct yt921x_priv *priv, int port, bool ingress, int to_local_port, struct netlink_ext_ack *extack)
{
	u32 srcs = ingress ? YT921X_MIRROR_IGR_PORTn(port) : YT921X_MIRROR_EGR_PORTn(port);
	u32 dst = YT921X_MIRROR_PORT(to_local_port);
	u32 ctrl, val;
	int res;
...
static void yt921x_dsa_port_mirror_del(struct dsa_switch *ds, int port, struct dsa_mall_mirror_tc_entry *mirror)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	struct device *dev = to_device(priv);
	int res;
...
static int yt921x_dsa_port_mirror_add(struct dsa_switch *ds, int port, struct dsa_mall_mirror_tc_entry *mirror, bool ingress, struct netlink_ext_ack *extack)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	int res;
...
static int yt921x_lag_hash(struct yt921x_priv *priv, u32 ctrl, bool unique_lag, struct netlink_ext_ack *extack)
{
	u32 val;
	int res;
...
static int yt921x_dsa_port_lag_check(struct dsa_switch *ds, struct dsa_lag lag, struct netdev_lag_upper_info *info, struct netlink_ext_ack *extack)
{
	...
	if (info->hash_type != NETDEV_LAG_HASH_L2 && info->hash_type != NETDEV_LAG_HASH_L23 && info->hash_type != NETDEV_LAG_HASH_L34) {
...
static int yt921x_dsa_port_lag_join(struct dsa_switch *ds, int port, struct dsa_lag lag, struct netdev_lag_upper_info *info, struct netlink_ext_ack *extack)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	struct dsa_port *dp;
	bool unique_lag;
	...
	case NETDEV_LAG_HASH_L34:
		ctrl |= YT921X_LAG_HASH_IP_DST | YT921X_LAG_HASH_IP_SRC |
			YT921X_LAG_HASH_IP_PROTO | YT921X_LAG_HASH_L4_DPORT | YT921X_LAG_HASH_L4_SPORT;
		break;
	case NETDEV_LAG_HASH_L23:
		ctrl |= YT921X_LAG_HASH_MAC_DA | YT921X_LAG_HASH_MAC_SA |
			YT921X_LAG_HASH_IP_DST | YT921X_LAG_HASH_IP_SRC | YT921X_LAG_HASH_IP_PROTO;
		break;
...
static int yt921x_fdb_has(struct yt921x_priv *priv, const unsigned char *addr, u16 vid, u16 *indexp)
{
	...
	if (res)
		return res;
	res = yt921x_reg_write(priv, YT921X_FDB_IN2, 0);
	if (res)
		return res;
...
static int yt921x_fdb_read(struct yt921x_priv *priv, unsigned char *addr, u16 *vidp, u16 *ports_maskp, u16 *indexp, u8 *statusp)
{
	struct device *dev = to_device(priv);
	u16 index;
	u32 data0, data1, data2, val;
	int res;
	...
	*statusp = FIELD_GET(YT921X_FDB_IO1_STATUS_M, data1);
	...
	dev_dbg(dev, "%s: index 0x%x, mac %02x:%02x:%02x:%02x:%02x:%02x, vid %d, ports 0x%x, status %d\n",
		__func__, *indexp, addr[0], addr[1], addr[2], addr[3], addr[4], addr[5], *vidp, *ports_maskp, *statusp);
	return 0;
}
...
static int yt921x_fdb_dump(struct yt921x_priv *priv, u16 ports_mask, dsa_fdb_dump_cb_t *cb, void *data)
{
	...
	ctrl = YT921X_FDB_OP_INDEX(0) | YT921X_FDB_OP_MODE_INDEX | YT921X_FDB_OP_OP_GET_ONE | YT921X_FDB_OP_START;
	res = yt921x_reg_write(priv, YT921X_FDB_OP, ctrl);
	if (res)
		return res;
	...
	index = 0;
	do {
		ctrl = YT921X_FDB_OP_INDEX(index) | YT921X_FDB_OP_MODE_INDEX | YT921X_FDB_OP_NEXT_TYPE_UCAST_PORT | YT921X_FDB_OP_OP_GET_NEXT | YT921X_FDB_OP_START;
		res = yt921x_reg_write(priv, YT921X_FDB_OP, ctrl);
		if (res)
			return res;
		res = yt921x_fdb_read(priv, addr, &vid, &pmask, &index, &status);
...
static int yt921x_fdb_add_index_in12(struct yt921x_priv *priv, u16 index, u16 ctrl1, u16 ctrl2)
{
	...
	if (res)
		return res;
	ctrl = YT921X_FDB_OP_INDEX(index) | YT921X_FDB_OP_MODE_INDEX | YT921X_FDB_OP_OP_ADD | YT921X_FDB_OP_START;
	res = yt921x_reg_write(priv, YT921X_FDB_OP, ctrl);
	if (res)
		return res;
...
static int yt921x_dsa_port_fdb_del(struct dsa_switch *ds, int port, const unsigned char *addr, u16 vid, struct dsa_db db)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	int res;
...
static int yt921x_dsa_port_fdb_add(struct dsa_switch *ds, int port, const unsigned char *addr, u16 vid, struct dsa_db db)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	int res;
...
static int yt921x_dsa_port_mdb_del(struct dsa_switch *ds, int port, const struct switchdev_obj_port_mdb *mdb, struct dsa_db db)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	int res;

	mutex_lock(&priv->reg_lock);
	res = yt921x_fdb_leave(priv, mdb->addr, mdb->vid, BIT(port));
	mutex_unlock(&priv->reg_lock);
	return res;
}

static int yt921x_dsa_port_mdb_add(struct dsa_switch *ds, int port, const struct switchdev_obj_port_mdb *mdb, struct dsa_db db)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	int res;

	mutex_lock(&priv->reg_lock);
	res = yt921x_fdb_join(priv, mdb->addr, mdb->vid, BIT(port));
	mutex_unlock(&priv->reg_lock);
	return res;
}
...
{
	u32 ctrl = !vlan_aware ? YT921X_PORT_IGR_TPIDn_STAG(0) : YT921X_PORT_IGR_TPIDn_CTAG(0);
...
{
	u32 mask = YT921X_PORT_VLAN_CTRL_CVID_M;
	u32 ctrl = YT921X_PORT_VLAN_CTRL_CVID(vid);
...
static int yt921x_vlan_filtering(struct yt921x_priv *priv, int port, bool vlan_filtering)
{
	...
	res = yt921x_reg_toggle_bits(priv, YT921X_VLAN_IGR_FILTER, YT921X_VLAN_IGR_FILTER_PORTn(port), vlan_filtering);
	if (res)
		return res;
...
{
	struct dsa_port *dp = dsa_to_port(&priv->ds, port);
	bool vlan_filtering = dsa_port_is_vlan_filtering(dp);
	u32 mask;
	int res;
...
static int yt921x_dsa_port_vlan_filtering(struct dsa_switch *ds, int port, bool vlan_filtering, struct netlink_ext_ack *extack)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	int res;
...
static int yt921x_dsa_port_vlan_del(struct dsa_switch *ds, int port, const struct switchdev_obj_port_vlan *vlan)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	u16 vid = vlan->vid;
	u16 pvid;
...
static int yt921x_dsa_port_vlan_add(struct dsa_switch *ds, int port, const struct switchdev_obj_port_vlan *vlan, struct netlink_ext_ack *extack)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	u16 vid = vlan->vid;
	u16 pvid;
	...
	do {
		...
		res = yt921x_vlan_add(priv, port, vid, vlan->flags & BRIDGE_VLAN_INFO_UNTAGGED);
		if (res)
			break;
		bdev = dsa_port_bridge_dev_get(dp);
		if (bdev) {
...
{
	u32 mask, ctrl = ~priv->cpu_ports_mask;
	int res;
...
{
	u32 mask = YT921X_PORT_LEARN_DIS;
	return yt921x_reg_clear_bits(priv, YT921X_PORTn_LEARN(port), mask);
}
...
static int yt921x_bridge_flags(struct yt921x_priv *priv, int port, struct switchdev_brport_flags flags)
{
	struct yt921x_port *pp = &priv->ports[port];
	bool do_flush = false;
	u32 mask;
	...
	if (do_flush) {
		...
		struct dsa_port *dp = dsa_to_port(ds, port);
		struct net_device *bdev = dsa_port_bridge_dev_get(dp);
		if (bdev) {
			u32 ports_mask;
			ports_mask = yt921x_bridge_ports_mask(ds, bdev);
...
static int yt921x_dsa_port_pre_bridge_flags(struct dsa_switch *ds, int port, struct switchdev_brport_flags flags, struct netlink_ext_ack *extack)
{
	if (flags.mask & ~(BR_HAIRPIN_MODE | BR_LEARNING | BR_FLOOD | BR_MCAST_FLOOD | BR_ISOLATED))
		return -EINVAL;
	return 0;
}

static int yt921x_dsa_port_bridge_flags(struct dsa_switch *ds, int port, struct switchdev_brport_flags flags, struct netlink_ext_ack *extack)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	int res;
...
static int yt921x_dsa_port_bridge_join(struct dsa_switch *ds, int port, struct dsa_bridge bridge, bool *tx_fwd_offload, struct netlink_ext_ack *extack)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	u16 ports_mask;
	int res;
	...
	if (dsa_is_cpu_port(ds, port))
		return 0;
	ports_mask = yt921x_bridge_ports_mask(ds, bridge.dev);
	ports_mask |= priv->cpu_ports_mask;
...
static int yt921x_dsa_port_mst_state_set(struct dsa_switch *ds, int port, const struct switchdev_mst_state *st)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	u32 mask, ctrl;
	int res;
...
static int yt921x_dsa_vlan_msti_set(struct dsa_switch *ds, struct dsa_bridge bridge, const struct switchdev_vlan_msti *msti)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	u32 masks[2], ctrls[2];
	int res;

	if (!msti->vid || !msti->msti || msti->msti >= YT921X_MSTI_NUM)
		return -EINVAL;
	masks[0] = 0;
	ctrls[0] = 0;
...
static int yt921x_port_up(struct yt921x_priv *priv, int port, unsigned int mode, phy_interface_t interface, int speed, int duplex, bool tx_pause, bool rx_pause)
{
	u32 mask, ctrl;
	int res;

	switch (speed) {
	case SPEED_10: ctrl = YT921X_PORT_SPEED_10; break;
	case SPEED_100: ctrl = YT921X_PORT_SPEED_100; break;
	case SPEED_1000: ctrl = YT921X_PORT_SPEED_1000; break;
	case SPEED_2500: ctrl = YT921X_PORT_SPEED_2500; break;
	case SPEED_10000: ctrl = YT921X_PORT_SPEED_10000; break;
	...
	if (yt921x_port_is_external(port)) {
		...
		switch (speed) {
		case SPEED_10: ctrl = YT921X_SERDES_SPEED_10; break;
		case SPEED_100: ctrl = YT921X_SERDES_SPEED_100; break;
		case SPEED_1000: ctrl = YT921X_SERDES_SPEED_1000; break;
		case SPEED_2500: ctrl = YT921X_SERDES_SPEED_2500; break;
		case SPEED_10000: ctrl = YT921X_SERDES_SPEED_10000; break;
		default: return -EINVAL;
		}
		...
		switch (speed) {
		case SPEED_10: ctrl = YT921X_MDIO_POLLING_SPEED_10; break;
		case SPEED_100: ctrl = YT921X_MDIO_POLLING_SPEED_100; break;
		case SPEED_1000: ctrl = YT921X_MDIO_POLLING_SPEED_1000; break;
		case SPEED_2500: ctrl = YT921X_MDIO_POLLING_SPEED_2500; break;
		case SPEED_10000: ctrl = YT921X_MDIO_POLLING_SPEED_10000; break;
		default: return -EINVAL;
		}
		if (duplex == DUPLEX_FULL)
...
static int yt921x_port_config(struct yt921x_priv *priv, int port, unsigned int mode, phy_interface_t interface)
{
	struct device *dev = to_device(priv);
	u32 mask, ctrl;
	int res;
	...
	case PHY_INTERFACE_MODE_2500BASEX:
		...
		case PHY_INTERFACE_MODE_100BASEX: ctrl = YT921X_SERDES_MODE_100BASEX; break;
		case PHY_INTERFACE_MODE_1000BASEX: ctrl = YT921X_SERDES_MODE_1000BASEX; break;
		case PHY_INTERFACE_MODE_2500BASEX: ctrl = YT921X_SERDES_MODE_2500BASEX; break;
		default: return -EINVAL;
		}
		res = yt921x_reg_update_bits(priv, YT921X_SERDESn(port), mask, ctrl);
		if (res)
...
static void yt921x_phylink_mac_link_down(struct dsa_switch *ds, int port, unsigned int mode, phy_interface_t interface)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	int res;
...
static void yt921x_phylink_mac_link_up(struct dsa_switch *ds, int port, unsigned int mode, phy_interface_t interface, struct phy_device *phydev, int speed, int duplex, bool tx_pause, bool rx_pause)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	int res;
...
static void yt921x_phylink_mac_config(struct dsa_switch *ds, int port, unsigned int mode, const struct phylink_link_state *state)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	int res;
...
static void yt921x_dsa_phylink_get_caps(struct dsa_switch *ds, int port, struct phylink_config *config)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
...
static enum dsa_tag_protocol yt921x_dsa_get_tag_protocol(struct dsa_switch *ds, int port, enum dsa_tag_protocol m)
{
...
static int yt921x_chip_detect(struct yt921x_priv *priv)
{
	...
	for (; info->name; info++)
		if (info->major == major && info->mode == mode && info->extmode == extmode)
			break;
